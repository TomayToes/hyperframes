...
description: >
  ...
  are identical and sacred — the green/pink/cream editorial triad, Cormorant Garamond at weight 500
...
typography:
  # — reading + chrome ramp —
  body:    { fontFamily: "Karla", cqw: 1.56, weight: 400, lineHeight: 1.38 }
  body-card:{ fontFamily: "Karla", cqw: 1.35, weight: 400, lineHeight: 1.34 }
  ...
  # — display ramp (Cormorant Garamond weight 500, opsz, negative tracking) —
  title-card-sm:{ fontFamily: "Cormorant Garamond", cqw: 2.9, weight: 500, lineHeight: 0.98, tracking: "-0.01em" }
  title-card:{ fontFamily: "Cormorant Garamond", cqw: 3.5, weight: 500, lineHeight: 0.96, tracking: "-0.01em" }
  headline:{ fontFamily: "Cormorant Garamond", cqw: 4.4, weight: 500, lineHeight: 1.0, tracking: "-0.02em" }
  headline-xl:{ fontFamily: "Cormorant Garamond", cqw: 5.0, weight: 500, lineHeight: 0.96, tracking: "-0.02em" }
  display:{ fontFamily: "Cormorant Garamond", cqw: 7.3, weight: 500, lineHeight: 1.02, tracking: "-0.02em" }
  display-hero:{ fontFamily: "Cormorant Garamond", cqw: 11.5, weight: 500, lineHeight: 0.92, tracking: "-0.02em" }
  stat-figure:{ fontFamily: "Cormorant Garamond", cqw: 11.5, weight: 500, lineHeight: 0.92, tracking: "-0.03em" }
  stat-figure-unit:{ fontFamily: "Cormorant Garamond", cqw: 5.7, weight: 500, lineHeight: 0.92 }
  name:    { fontFamily: "Cormorant Garamond", cqw: 2.3, weight: 600, lineHeight: 1.0 }
...
- **Fonts** — **Cormorant Garamond** (display, weight 500) / **Karla** (body + card copy) / **JetBrains Mono** (chrome: labels, counters, units, footlines). Ignore any preset font name lingering in prose.
- **Colors — CEMESA "tech"/dark identity. Read every preset color NAME in prose through this remap:**
  - `green` `#1a662d` = **CEMESA emerald** (primary brand green); `green-deep` `#004b1c`; `green-lite` `#2f9e57` (brighter emerald — scan glow / lit zones).
  - `pink` `#c0a77e` / `pink-deep` `#9c7f56` = **champagne GOLD** (the warm secondary accent — figures, the twist highlight, premium moments). Wherever the prose says "pink", render champagne gold.
  - `cream` `#f7f3e9` = warm-white ink / light ground; `cream-2` `#e7dcc4` = pale champagne tint.
  - `ink` `#080f09` = **deep green-black canvas.** This is a DARK-leaning promo: prefer `ink` and `green` grounds for the reveal (the preset's cream default yields to dark here); cream is used for text and the occasional bright plate, not as the dominant ground.
- **Triad = emerald green + champagne gold + warm cream, on a green-black canvas.** Two surface tones max per frame still holds.


## Overview

Editorial Forest at frame scale is a **serif-led literary-editorial system** — the register of a
Penguin classic or a quiet annual report. One confident voice, **Cormorant Garamond at weight 500**
...
- **Cormorant Garamond weight 500** (opsz, negative tracking) for all display; **serif 400** body; **JetBrains Mono 500** uppercase chrome.
...
- **Squint** — one Cormorant Garamond moment dominates at 3–6× its neighbor; the surface block centers the eye.
...
Two ramps. The **reading/chrome ramp** (Cormorant Garamond **400** body 1.56cqw; JetBrains Mono labels
in px) carries copy + chrome; the **display ramp** (Cormorant Garamond **500** `title-card-sm` 2.9cqw →
...
**Ground** `{colors.green}`, `slide-pad`. **Composes** topbar (mono label + monogram-circle), display-hero, body lede, footline. **Focal** a 2-line Cormorant Garamond `display-hero` in pink (one word in cream), left. **Chrome** mono topbar; mono footline. **Accent** pink display + the monogram circle. **Silence** ~50%. **Fixed** serif 500, flat, monogram 2px pink. **Free** title, which word is cream. **Density** low.
...
**Ground** `{colors.green}`, `slide-pad`. **Composes** topbar, kpi-block (mono tag + 220px stat-figure + serif description), footline. **Focal** a Cormorant Garamond `stat-figure` (~11.5cqw, +unit) in pink over a 2px pink rule. **Chrome** mono topbar + footline. **Accent** the pink figure. **Silence** ~55%. **Fixed** serif 500 figure, 2px accent rule, flat. **Free** the figure (from script), tag, description. **Density** low.

### 4 · Statement (quote · move: display serif · cream · left)

**Ground** `{colors.cream}`, generous pad. **Composes** topbar, display, name (600), caption-mono role. **Focal** a 2-line Cormorant Garamond `display` (~7.3cqw) in green; a 600-weight attribution name + mono role beneath. **Chrome** mono topbar. **Accent** none — the serif carries it. **Silence** ~55%. **Fixed** serif 500 display + 600 name only, no italic. **Free** quote, name, role. **Density** low.
...
- Run every display in **Cormorant Garamond weight 500**, opsz, negative-tracked, tight line-height; **body at 400**.
...
- **Type** — Cormorant Garamond **500** display (opsz, negative-tracked) / **400** body; mono uppercase 0.08–0.18em; ≥1.4cqw floor.
...
- **Cormorant Garamond (opsz 8..60) + JetBrains Mono via Google Fonts** — the optical-size axis is critical; a non-opsz fallback flattens the size-aware letterforms. CJK: LXGW WenKai (display) / Noto Serif SC (body) / Noto Sans Mono CJK (chrome).
